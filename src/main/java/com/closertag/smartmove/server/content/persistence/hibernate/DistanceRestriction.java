...
private String alias;
	 ...
	 */
	public DistanceRestriction(Point point,String alias, double distance, boolean useIndex) {
		this.point = point;
	 ...
	 */
	public DistanceRestriction(Point point, String alias, double distance) {
		this.point = point;
		this.distance = distance;
		this.alias = alias;
	}
	...
	public TypedValue[] getTypedValues(Criteria criteria,
		...
		return new TypedValue[] { criteriaQuery.getTypedValue(criteria,
				"gpsPositions."+GpsPosition.LOCATION_COLUMN_NAME, point) };
	...
	public String toSqlString(Criteria criteria, CriteriaQuery criteriaQuery)
		...
		String[] columnName = criteriaQuery.getColumnsUsingProjection(criteria,  alias+"."+GpsPosition.LOCATION_COLUMN_NAME);
	
		
		StringBuffer result = new StringBuffer("( distance_sphere(").append(
				columnName[0]).append(", ?) <=").append(this.distance).append(")");
		return useIndex ? result.append(" AND ").append(
				getBoundingBox(criteriaQuery.getSQLAlias(criteria, alias+"."+GpsPosition.LOCATION_COLUMN_NAME), this.point
	...
	private String getBoundingBox(String alias, double lat, double lon,
		...
		sb.append(alias);
		sb.append(".");
		sb.append(GpsPosition.LOCATION_COLUMN_NAME);
		sb.append(" ");
