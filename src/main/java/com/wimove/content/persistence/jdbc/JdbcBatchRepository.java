 ...
 * 
 */
package com.wimove.content.persistence.jdbc;
...
import com.wimove.content.domain.AbstractLabelValueModel;
...
import com.wimove.content.persistence.BatchRepository;
...
public class JdbcBatchRepository implements BatchRepository {
	...
	//SELECT i.item_id,li.locale  FROM item i, localizeditem li WHERE i.id=li.item_id AND  li.label='Description' AND li.value<>''  AND not exists (SELECT id from localizeditem li_ WHERE li_.item_id= li.item_id AND  li_.label='Mp3' and li_.locale = li.locale)   LIMIT 1
				...
				jdbcTemplate.batchUpdate(INSERT_INTO_ITEM_TABLE,
						new BatchPreparedStatementSetter() {
							public void setValues(PreparedStatement ps, int i)
								...
								ps.setObject(8, new java.sql.Date(new Date()
										.getTime()));
								ps.setDate(9, new java.sql.Date(new Date()
										.getTime()));
				...
				jdbcTemplate.batchUpdate(UPDATE_INTO_ITEM_TABLE,
						new BatchPreparedStatementSetter() {
							public void setValues(PreparedStatement ps, int i)
								...
								ps.setString(6, item.getTelephone());
								ps.setString(7, item.getEmail());
								ps.setDate(8, new java.sql.Date(new Date()
										.getTime()));
								ps.setDate(9, new java.sql.Date(new Date()
										.getTime()));
								ps.setLong(10, id);
	...
	@SuppressWarnings("unchecked")
	public List<HashMap<String, String>> getItemsWithoutLocale(int howmany) {
	...
	private void insertLocalizedItem(final Locale locale, final Item item) {
		...
		jdbcTemplate
				.update(
						"DELETE FROM localizeditem WHERE item_id=? and locale=? and label<>'Mp3'",
